...
public class PurchaseManagerPagingVO {
	...
	public PurchaseManagerPagingVO() {}
	
//	PagingVO(12, 1, 5)
	public PurchaseManagerPagingVO(int total, int nowPage, int cntPerPage) {
		setNowPage(nowPage);
		setTotal(total);
		setCntPerPage(cntPerPage);
		calcLastPage(total, cntPerPage); //calcLastPage(12, 5);
		calcStartEndPage(nowPage, cntPage); //calcStartEndPage(1, 5);
		calcStartEnd(nowPage, cntPerPage); //calcStartEnd(1, 5);
	}
	
	// 제일 마지막 페이지 계산
	public void calcLastPage(int total, int cntPerPage) { // 12, 5
	...
	public void calcStartEndPage(int nowPage, int cntPage) {//1,5
		...
		setEndPage(getStartPage()+cntPage-1);//setEndPage(5);
		
		if (getLastPage() < getEndPage()) { //if (2 < 5) 
			setEndPage(getLastPage()); //setEndPage(2);
		}
	}
	
	// DB 쿼리에서 사용할 start 값 계산
	public void calcStartEnd(int nowPage, int cntPerPage) { //1,5
